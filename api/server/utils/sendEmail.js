...
async function getEmailBranding() {
  if (!brandingService) {
    brandingService = require('~/server/services/BrandingService');
  }
  try {
    return await brandingService.getBrandingForEmail();
  } catch {
    return { headerColor: '#16a34a', footerText: 'LibreChat', logoUrl: '', appName: 'LibreChat' };
  }
}

const sendEmail = async ({ email, subject, payload, template, throwError = true, html: rawHtml }) => {
  try {
    let html;
    if (rawHtml) {
      html = rawHtml;
      ...
      throw new Error('Either template+payload or rawHtml must be provided');
    }

    // Inject branding into HTML if applicable
    if (!rawHtml) {
      const emailBranding = await getEmailBranding();
      const brandingCss = `<style>.brand-header { background-color: ${emailBranding.headerColor}; } .brand-footer { color: #9ca3af; font-size: 12px; text-align: center; padding: 20px; }</style>`;
      if (typeof html === 'string' && !html.includes('brand-header')) {
        html = html.replace('</head>', `${brandingCss}</head>`);
        html = html.replace('</body>', `<div class="brand-footer">${emailBranding.footerText}</div></body>`);
      }
    }
