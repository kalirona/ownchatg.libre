...
const startServer = async () => {
  ...
  app.get('/sitemap.xml', (_req, res) => {
    const pages = [
      ...
      { loc: '/legal/privacy', priority: '0.5', changefreq: 'yearly' },
      { loc: '/legal/terms', priority: '0.5', changefreq: 'yearly' },
      { loc: '/legal/cookies', priority: '0.5', changefreq: 'yearly' },
      { loc: '/legal/dpa', priority: '0.5', changefreq: 'yearly' },
      { loc: '/sitemap', priority: '0.4', changefreq: 'monthly' },
    ];
    const urls = pages
      .map(
        (p) => `<url><loc>https://ownchatgptbusiness.com${p.loc}</loc><priority>${p.priority}</priority><changefreq>${p.changefreq}</changefreq></url>`,
      )
      .join('\n      ');
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  app.use(noIndex);
  /* Webhook routes must be mounted BEFORE express.json() to capture raw body for signature verification */
  ...
  /* CORS — fail closed in production: ALLOWED_ORIGINS is required */
  if (isProduction) {
    ...
    if (allowedOrigins.length === 0) {
      console.error(
        'FATAL: ALLOWED_ORIGINS is not set. CORS would deny all cross-origin requests. ' +
  ...
  app.use(require('~/server/middleware/securityHeaders'));

  /* Global rate limiter (catch-all safety net) */
  app.use(require('~/server/middleware/limiters/globalLimiter'));
  ...
  app.use('/api/admin/super', routes.adminSuper);
  app.use('/api/cost-optimizer', routes.costOptimizer);
  app.use('/api/prompt-optimizer', routes.promptOptimizer);
  app.use('/api/agent-marketplace', routes.agentMarketplace);
  ...
  app.use('/api/branding', routes.branding);
  app.use('/api/workflows', routes.workflows);
  app.use('/api/invites', routes.invites);
  app.use('/status', routes.uptime);
  app.use('/api/better-stack', routes.betterStack);
  app.use('/api/organizations', routes.sharedFolders);
  app.use('/api/organizations', routes.teamPrompts);
  app.use('/api/organizations', routes.teamAgents);
  app.use('/api/organizations', routes.orgBilling);
  ...
  const server = app.listen(port, host, async (err) => {
    ...
    try {
      ...
      if (isEnabled(process.env.MONITORING_ENABLED)) {
        monitoringService.startPeriodicChecks();
      }
