...
const updateProfileController = async (req, res) => {
  try {
    const { name, timezone, theme, language, preferences } = req.body;
    const updateData = {};
    if (name !== undefined) {
    ...
    const user = await db.updateUser(req.user.id, updateData);
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }
    res.status(200).send(sanitizeUserForResponse(user));
  } catch (error) {
